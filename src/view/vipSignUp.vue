    ...
    <div class="signUpPCPage">
        ...
        <div class="audience-registration-box shuiyin-bg-box" id="audience">
            <div class="audience-registration">
                ...
                <!-- <div class="deadline">（报名截止时间：12月1日 17:00）</div> -->
                <div class="list">
                    ...
                    <div class="mobile" v-show="hideSlect && allShow">
                        ...
                        <label for="email" style="margin-right:.8vw">抽签序号:</label>
...
export default {
    ...
    mounted(){
        ...
        if (this._isMobile()) {
            this.h5Show = true;
            this.pcShow = false;
            this.$router.push({path:'/vipSignUpH5',query: {name: 8,id:this.matchtypey}});
        } else {
            this.h5Show = false;
            this.pcShow = true;
        };
                ...
                if(res.status == 200 ){
                    if(res.data && res.data.success && res.data.success != 'false'){
                        if(that.isNewa){
                            that.$notify({
                                title: '成功',
                                message: '请等待工作人员审核，审核成功将有邮件通知,未收到邮件和收到审核拒绝邮件请勿前往参加。',
                                type: 'success'
                            });
                        }else{
                            that.$notify({
